...
export default function BlogListClient({ allPosts }: { allPosts: BlogPostData[] }) {
    ...
    return (
        <div className="py-12 px-4 md:px-0 max-w-3xl mx-auto w-full">
            <FadeIn delay={0.1}>
                <h1 className="text-4xl font-bold tracking-tight text-current opacity-90 mb-4">
                    {t({ ru: "Блог", en: "Blog" })}
                </h1>
            ...
            <div className="flex flex-col gap-6">
                {posts.map((post, idx) => (
                    <FadeIn key={post.id} delay={0.2 + idx * 0.1}>
                        <Link
                            href={`/blog/${post.id}`}
                            className="group block p-6 rounded-3xl bento-card hover:bg-current/10 transition-colors"
                        >
                            <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-2 mb-3">
                                <h2 className="text-2xl font-bold text-current opacity-90 group-hover:text-indigo-400 transition-colors">
                                    {post.title}
                                </h2>
                                <time className="text-sm opacity-50 whitespace-nowrap">{post.date}</time>
                            </div>
                            <p className="opacity-60 mb-4 line-clamp-2">{post.excerpt}</p>
                ...
                {posts.length === 0 && (
                    <p className="opacity-50">{t(siteConfig.ui.blog.empty)}</p>
                )}
            </div>
        </div>
    );
