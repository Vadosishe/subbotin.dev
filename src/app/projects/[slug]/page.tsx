...
export async function generateMetadata(props: Params): Promise<Metadata> {
    ...
    return {
        title: project ? `${project.title} | Влад Субботин` : "Проект не найден",
...
const statusLabels: Record<string, { text: string; color: string }> = {
    active: { text: "Актив", color: "bg-green-500/20 text-green-400" },
    beta: { text: "Бета", color: "bg-yellow-500/20 text-yellow-400" },
    archived: { text: "Архив", color: "bg-gray-500/20 text-gray-400" },
};
        ...
        <article className="py-12 px-4 md:px-0 max-w-3xl mx-auto w-full">
            <FadeIn delay={0.1}>
                <div className="mb-8">
                    ...
                    <p className="text-lg" style={{ color: 'var(--muted)' }}>{project.description}</p>
            ...
            <FadeIn delay={0.2} className="mt-10">
                <div
                    ...
                    dangerouslySetInnerHTML={{ __html: simpleMarkdown(project.longDescription) }}
                />
            </FadeIn>
        </article>
    );
