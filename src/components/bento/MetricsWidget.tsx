...
export function MetricsWidget() {
    ...
    return (
        <div className="flex flex-col h-full p-6 bg-gradient-to-br from-white/[0.02] to-transparent relative overflow-hidden group">
            <h2 className="text-lg font-bold mb-6 flex items-center gap-2 relative z-10 text-white/90">
            ...
            <div className="grid grid-cols-2 gap-y-6 gap-x-4 flex-grow relative z-10">
                {siteConfig.metrics.map((metric, idx) => (
                    <div key={idx} className="flex flex-col justify-center">
                        <div className="text-3xl lg:text-4xl font-black tracking-tight text-white/90 flex transition-transform group-hover:scale-[1.02] origin-left items-baseline">
                            <AnimatedNumber value={metric.value} />
                            <span className="text-white/40 ml-1 text-xl font-bold">{metric.suffix}</span>
                        </div>
                        <p className="text-xs md:text-sm font-medium text-white/50 uppercase tracking-widest mt-1">
                            {t(metric.label)}
                        </p>
                    </div>
                ))}
